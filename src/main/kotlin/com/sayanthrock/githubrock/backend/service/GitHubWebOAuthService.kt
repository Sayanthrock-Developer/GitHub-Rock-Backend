...
internal const val GITHUB_ROCK_WEB_OAUTH_SCOPES = "repo workflow read:user user:email read:org notifications user:follow"

class GitHubWebOAuthService(private val config: AppConfig, private val client: HttpClient) {
    val isConfigured: Boolean get() = config.githubOauthClientId.isNotBlank() && config.githubOauthClientSecret.isNotBlank()
    val callbackUrl: String get() = "${config.publicBaseUrl.trimEnd('/')}/v1/auth/github/callback"
    ...
    fun authorizationUrl(state: String, codeChallenge: String): String {
        ...
        require(codeChallenge.length in 43..128) { "Invalid OAuth code challenge" }
    ...
    suspend fun exchange(code: String, codeVerifier: String): GitHubTokenResponse {
        require(isConfigured) { "GitHub OAuth web flow is not configured" }
        require(code.length in 10..4096) { "Invalid authorization code" }
        require(codeVerifier.length in 43..128) { "Invalid OAuth code verifier" }
        return client.submitForm("https://github.com/login/oauth/access_token", Parameters.build {
        ...
        }) { headers { append(HttpHeaders.Accept, "application/json") } }.body()
    }
}
